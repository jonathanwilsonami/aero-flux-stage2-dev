...
<!-- TODO: State the specific problem this project solves, in your own framing.
The one to sharpen: it's NOT "predict flight delays" (well-trodden). It's
"perceive cascading state in a tightly-coupled physical system in real time,
anticipate propagation, and support action before the window closes — with
flight delay as the proof-of-concept." Keep it 1-2 paragraphs. This is the
paragraph that tells the reader why the project is more than a delay model. -->

_TODO: problem statement._

## Contribution {#sec-contribution}

<!-- TODO: THE most important section for novelty. Argue what AeroFlux
contributes BEYOND flight-delay prediction. Anchor on:
 - Propagation-aware modeling of cascading state (rotation as one nullable
   channel over an always-available demand/weather/schedule backbone).
 - The Swiss Cheese Model (Reason) framing — seeing the holes align in time to act.
 - Generalization toward cyber-physical intelligence (sensors, robotics,
   multi-domain propagation) — flight delay is the instance, not the point.
 - The engineering contribution: train/serve parity by construction, a portable
   local<->cloud platform, honest live evaluation of a streaming predictor.
State clearly what is genuinely new vs. what is standard. -->

_TODO: contribution — the "why should anyone care" argument._

## Scope

The long-term vision for AeroFlux is ambitious and continues to evolve. This
project focuses on building a scalable, modular, configurable, portable, and
decoupled big-data platform that can ingest, fuse, process, and analyze aviation
and weather data in real time, establishing the technical foundation for future
AeroFlux capabilities. Flight-delay prediction serves as the primary use case,
but the architecture is designed to support additional applications, including
congestion analysis, delay-propagation modeling, operational monitoring,
decision support, and agentic AI. Stage 1 established a historical
machine-learning pipeline and interactive prototype [@wilson2026flightdelay];
this work extends that with a live data platform, baseline-vs-advanced model
comparison, cloud deployment, live-prediction evaluation, and an explainable
analyst agent prototype.
